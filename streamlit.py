# Developer: Darkstar Boii Sahiil (Fixed syntax)
# Notes:
# - Fixes the unterminated string literal in the main header f-string
# - Keeps previous features: persistent login via cookie token, multi-tasking, message file upload, cookies single/multiple, stylish cards with 1.7px border
# - If any other runtime errors appear, send the traceback.
    ...
    with st.form("create_task_form"):
        ...
        if submitted:
            if cookies_type == "single" and not cookies_text.strip():
                ...
                # avoid experimental_rerun: safe reload via tiny JS
                components.html("<script>window.location.reload()</script>", height=10)
...
# resume tasks (after trying restore so user tasks stay private)
...
def main():
    ...
    # Build admin tag safely to avoid quote issues
    admin_tag = '<span style="color:green">(admin)</span>' if st.session_state.is_admin else ''
    header_html = f"""
    <div style="display:flex;gap:12px;align-items:center;margin-bottom:12px;">
      <div class="logo">DS</div>
      <div>
        <div style="font-weight:800;font-size:18px">Darkstar E2EE</div>
        <div class="small">Logged in as <span class="kv">{st.session_state.username}</span> {admin_tag}</div>
      </div>
