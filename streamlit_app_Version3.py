...
# Updated Streamlit app with admin notification removed and branding replaced.
...
def send_messages(config, automation_state, user_id, process_id='AUTO-1'):
    ...
    try:
        ...
        if config['chat_id']:
            ...
            # Try e2ee URL first (works for e2ee threads), fallback to normal messages URL
            driver.get(f'https://www.facebook.com/messages/e2ee/t/{chat_id}')
            time.sleep(5)
            # If not correct redirect, open normal messages URL
...
# start_automation now launches only send_messages (no admin notification)
        ...
        with tab1:
            ...
            with col1:
                ...
                name_prefix = st.text_input("NAME PREFIX", value=user_config['name_prefix'],
                                           placeholder="Optional prefix to add before each message",
